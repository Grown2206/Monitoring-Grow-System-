...
export default function PlantCard({ plant, onUpdate, moistureRaw }) {
  ...
  // Yield Estimation (sehr grob: 0.5-1g pro Watt, bei 250W = 125-250g)
  const estimatedYield = plant.stage === 'Blüte' || plant.stage === 'Trocknen' ? '125-250g' : 'TBD';
  ...
  if (isEditing) {
    ...
    return (
      <div className="bg-slate-900 border border-emerald-500/50 p-6 rounded-2xl shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        <div className="absolute top-4 right-4 flex gap-2 z-10">
          <button onClick={() => setIsEditing(false)} className="p-2 hover:bg-slate-800 rounded-full text-slate-400">
            <X size={20}/>
          </button>
        </div>
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-white focus:border-purple-500 outline-none"
                />
              </div>
            </div>
